...
export function parseStructuredResponse(response) {
  ...
  try {
    ...
    // Hitting the output cap truncates the JSON mid-structure, so the parse
    // failure is a symptom. Say so — "invalid JSON" sends you looking in the
    // wrong place, and the fix here is a smaller input or a higher cap.
    if (response.stop_reason === 'max_tokens') {
      return {
        error: 'That PDF produced more notes than fit in one response. Try a shorter section.',
        status: 413
      };
    }
